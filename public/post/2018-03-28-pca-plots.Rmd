...
```{r eval=FALSE}
if (clusters != 1 || autoClustering) { # Case 1: If either the user specifies a number of clusters or automatic clustering, then this part of code is triggered
...
samplePCA <- function(expressionMatrix,numberOfPCs=2,clusters=1,autoClustering=FALSE,colorByCluster=FALSE){
  pca <- stats::prcomp(t(expressionMatrix),center=TRUE,scale=TRUE)
  ...
  if (autoClustering) {
    clusterNumber <- NULL # NULL is the value eclust prefers for automatic cluster optimization
  } else {
    clusterNumber <- clusters
  }
  
  if (clusters != 1 || autoClustering) { # If either the user specifies a number of clusters or automatic clustering, then this part of code is triggered
    # Using factoextra's eclust to find clusters
    pdf(NULL) # This is just to suppress the graphical output for the moment, but we wanna save this plot later
    eclustering <- factoextra::eclust(
      t(expressionMatrix),
    ...
    if(colorByCluster){
      graphingParameters <- list(mapping=aes(col=Cluster)) #User has a choice to color by cluster
    } else{
      graphingParameters <- list(mapping=aes(col=Sample))
    }
  }
